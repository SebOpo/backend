...
async def serialize_excel(spreadsheet: Worksheet) -> Dict:
    locations = []
        ...
        try:
            location = {
                ...
                "reports": {
                    ...
                    "electricity": {"flag": row[6].lower(), "description": ""},
                    "carEntrance": {"flag": row[7].lower(), "description": ""},
                    "water": {"flag": row[8].lower(), "description": ""},
                    "fuelStation": {"flag": row[9].lower(), "description": ""},
                    "hospital": {"flag": row[10].lower(), "description": ""},
                },
            }
            ...
            unprocessed_locations.append(
                {"location": row, "code": "SERIALIZATION_ERROR", "detail": e}
            )

    return {"processed": locations, "unprocessed": unprocessed_locations}
...
async def geocode_locations(locations: List[Dict]) -> Dict:
    ...
    for location in locations:
        address_string = "{}, {}".format(
            location.get("address"), location.get("street_number")
        )
        coordinates = geocode_address(address=address_string, city=location.get("city"))
        if not coordinates:
            unprocessed_locations.append(
                {
                    "location": location,
    ...
    return {"geocoded": geocoded_locations, "unprocessed": unprocessed_locations}
