...
def test_invite_existing_user(
    ...
    payload = {"email": settings.FIRST_SUPERUSER, "organization": dim_org.id}
...
def test_confirm_user_invite(client: TestClient, test_db: Session) -> None:
    test_user_registration_token = crud.get_by_email(
    ...
    r = client.post(
        f"{settings.API_V1_STR}/users/confirm-registration?access_token={test_user_registration_token}",
        json=payload,
    )
...
def test_patch_user(
    ...
    r = client.put(
        f"{settings.API_V1_STR}/users/info",
        json=payload,
...
def test_patch_user_password(
    ...
    incorrect_login_request = client.post(
        f"{settings.API_V1_STR}/auth/login/token", data=incorrect_login_payload
    )
    ...
    correct_login_request = client.post(
        f"{settings.API_V1_STR}/auth/login/token", data=correct_login_payload
    )
...
def test_user_password_reset(
    ...
    login_request = client.post(
        f"{settings.API_V1_STR}/auth/login/token", data=login_payload
    )
