...
async def restrict_zone(
    ...
    existing_zone = crud.zones.get_zone_by_verbose_name(db, zone.verbose_name)
    if existing_zone:
        raise HTTPException(
    ...
    restricted_zone = crud.zones.create(
        db=db, obj_in=schemas.ZoneCreate(**zone.dict(), bounding_box=str(geom))
    )
...
async def allow_zone(
    ...
    result = crud.zones.delete(db=db, model_id=zone_id)
