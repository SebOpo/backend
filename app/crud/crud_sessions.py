...
def get_by_access_token(
    ...
    return (
        ...
        .filter(
            SessionHistory.access_token == access_token,
...
def get_user_active_sessions(db: Session, *, user_id: int) -> List[SessionHistory]:
    return (
        db.query(SessionHistory)
        .filter(SessionHistory.user_id == user_id, SessionHistory.is_active == True)
        .all()
    )
...
def revoke_by_id(db: Session, *, user_id: int, session_id: int) -> SessionHistory:
    db_obj = (
        db.query(SessionHistory)
        .filter(SessionHistory.user_id == user_id, SessionHistory.id == session_id)
        .first()
    )
    if not db_obj:
        return None
